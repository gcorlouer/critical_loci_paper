...
\begin{definition}[Residual variety $\Gamma_d^r$]
...
\begin{equation*}
    \Sigma_d^r := \{W\in\Omega| \text{rk}(W_L...W_1) = r\}
\end{equation*}
We can relate $\Sigma_d^r$ to the residual variety $\Sigma_{d-r}^0$:
\begin{equation*}
...
\begin{align*}
    \Sigma_d^r = GL(d).\iota_r(\Sigma_{d-r}^0) \\
    g.W = (g_L W_L g_{L-1}^{-1}, \dots, g_1 W_1 g_0^{-1}) \\
    \iota_r(Z)_\ell = \begin{pmatrix} I_r & 0 \\ 0 & Z_\ell \end{pmatrix}
\end{align*}
We will be interested in the following space of residuals with dimension vector $d=(d_0, d_1-r\dots,d_L-r)$:
\[
\Sigma^0_d
:=
\left\{
...
\end{definition}
\begin{definition}
Define the algebraic variety of residuals satisfying the cyclic vanishing conditions:
\[
\Gamma_{d,XY}:= \Sigma^0_d\cap\Gamma_d^r
\]
\end{definition}
\begin{comment}
...
\end{equation}
\end{comment}
\begin{definition}[Action of $G_d$] Define a group action of $G_d = \prod_{\ell=1}^{L-1} GL(d_\ell,\R)$ on $\Omega$ by:
\begin{align*}
...
\begin{equation*}
    \mathcal{C}_r = \{W\in \Omega| \exists Z\in\Gamma_{d,XY}, W_L \sim [U_S, U_QZ_L],  W_l \sim \begin{pmatrix} I_r & 0 \\
                                                      0.  & Z_l      
                                        \end{pmatrix}, W_1 \sim [\Sigma_X^{-1}\Sigma_{XY}U_S, Z_1^{\top}]^{\top}\}
\end{equation*} 
In other words, $\mathcal{C}_r \subset \Omega$ denote the set of rank-$r$ first-order critical points in a normal form of the DLN, i.e.\ the set described by the existence of residual blocks $Z = (Z_1, \dots, Z_L)\in \Gamma_{d,XY}$ such that there exists $g \in G_d$ with
\[
...
In this case the cyclic residual variety $\Gamma_{d,XY}$ reduces to the residual
representation space of an equioriented type-$A$ quiver with dimension vector
\[
(d_0,d_1-d_L,\dots,d_{L-1}-d_L),
\]
...
\end{proposition}
%------------------------------------------------------------
\section{Orbits of Critical loci as orbits of $\Gamma_{d,XY}$}
%------------------------------------------------------------
...
\begin{equation}\label{eq:Scrit-def}
 S_{r,\sigma} \;:=\; \iota(\Gamma_{d,XY}) \;\subset\; \Omega .
\end{equation}

\begin{definition}[Hidden-layer gauge group and its slice stabilizer.]
Define the subgroup $H_{r,\sigma}\le G_d$ consisting of those hidden reparametrizations that preserve the block decomposition
...
\;:=\;
\left\{
h=(h_1,\dots,h_{L-1})\in G_d \;\middle|\;
h_\ell=
\begin{pmatrix}
A & 0\\
0 & K_\ell
...
\right\}.
\end{equation}
(Equivalently: $H_{r,\sigma}$ is the setwise stabilizer of $\mathcal S_{r,\sigma}$ inside $G_d$).
\noindent
...
\qquad (\ell=2,\dots,L-1),
\end{equation}
with the endpoint conventions for $\ell=1$ and $\ell=L$ determined by the action of $G_d$ over the fixed endpoint embeddings
\eqref{eq:endpoints-slice}.
By construction, $\Gamma_{d,XY}\subset \mathcal Z$ is $H_{r,\sigma}$-stable (since
$h\cdot \mathcal S_{r,\sigma}=\mathcal S_{r,\sigma}$).
\end{definition}
\noindent
\begin{proposition}\label{prop:orbit-saturation}
The critical loci $\mathcal{C}_r$ is the orbit of the gauge-fixed critical slice $\mathcal{S}_{r,\sigma}$ by the hidden-layer gauge group $G_d$:
\begin{equation}\label{eq:orbit-saturation}
\mathcal{C}_r = G_d \cdot S_{r,\sigma}.
\end{equation}
\end{proposition}
\noindent
We prove this proposition in the appendix \ref{proof:orbit-saturation}
\begin{proposition}\label{prop:parameters-as-bundle}
    Define $\Phi \colon G_d \times \Gamma_{d,XY} \to \Omega$ by
\begin{equation}
\Phi(g, Z) := g \cdot \iota(Z),
\end{equation}
Then $\Phi(G_d \times \Gamma_{d,XY}) = \mathcal{C}_r$ and $\Phi$ is invariant under the right action of $H_{r,\sigma}$ on $G_d \times \Gamma_{d,XY}$ given by
\begin{equation}
(g, Z) \cdot h := (g h,\; h^{-1} \cdot Z).
\end{equation}
Hence $\Phi$ descends to a surjective map
\[\
\bar\Phi \colon G_d \times_{H_{r,\sigma}} \Gamma_{d,XY} \longrightarrow \mathcal{C}_r.
\]
With 
\begin{equation*}
     G_d \times_{H_{r,\sigma}} \Gamma_{d,XY} := (G_d \times \Gamma_{d,XY}) / H_{r,\sigma}
\end{equation*}
\end{proposition}
\noindent
We prove this proposition in the appendix \ref{proof:parameters-as-bundle}.
\begin{definition}
Define the \emph{transporter into the slice} at a point $Z' \in \Gamma_{d,XY}$ by
\[
\mathrm{Trans}_{G_d}\!\bigl(\iota(Z'),\, S_{r,\sigma}\bigr)
:= \bigl\{ g \in G_d : g \cdot \iota(Z') \in S_{r,\sigma} \bigr\}.
\]
Define the \emph{principal slice locus}
\[
\Gamma_{d,XY}^{\circ} := \bigl\{ Z' \in \Gamma_{d,XY} \;\big|\;
\mathrm{Trans}_{G_d}\!\bigl(\iota(Z'),\, S_{r,\sigma}\bigr) = H_{r,\sigma} \bigr\},
\]
and let $\mathcal{C}_r^{\circ} := \Phi(G_d \times \Gamma_{d,XY}^{\circ})$.
\end{definition}
\begin{theorem}\label{thm:critical-bundle}
The restriction
\[
\bar\Phi \colon G_d \times_{H_{r,\sigma}} \Gamma_{d,XY}^{\circ}
\xrightarrow{\;\sim\;} \mathcal{C}_r^{\circ}
\]
is a bijection.  In particular, on the principal slice locus one has the identification:
\begin{equation}
\mathcal{C}_r^{\circ} \;\cong\; G_d \times_{H_{r,\sigma}} \Gamma_{d,XY}^{\circ}
...
\begin{equation}
\mathcal{C}_r^{\circ} / G_d \;\cong\; \Gamma_{d,XY}^{\circ} / H_{r,\sigma}.
\end{equation}
\end{theorem}
\noindent
The proof of the latter theorem can be found appendix \ref{app:loci-cyclic}.
We will use cyclic quiver representation theory to stratify $\Gamma_{d,XY}$ by rank data and then use the above bundle structure to stratify the critical loci $\mathcal C_r$.
\begin{remark}[Why a rank-stratification of $\Gamma_{d,XY}$ does not lift naively to $\mathcal C_r$]
A priori, the orbit decomposition of $\Gamma_{d,XY}$ by rank data does \emph{not} induce a disjoint decomposition of the critical locus $\mathcal C_r$ by the same labels. The reason is that Proposition~3.4 only provides a surjective map
\[
\bar\Phi \colon G_d \times_{H_{r,\sigma}} \Gamma_{d,XY} \longrightarrow \mathcal C_r,
\]
whereas injectivity is established only after restricting to the principal slice locus $\Gamma^\circ_{d,XY}$ in Theorem~3.6.
\noindent
Concretely, let $O_{\rho,\sigma}, O_{\rho',\sigma} \subset \Gamma_{d,XY}$ be two distinct $G_\sigma$-orbits, corresponding to different rank strata. In order for these to define distinct strata in $\mathcal C_r$, one would need that
\[
G_d \cdot \iota(O_{\rho,\sigma}) \cap G_d \cdot \iota(O_{\rho',\sigma}) = \varnothing
\qquad\text{whenever } \rho \neq \rho'.
\]
However, this need not hold outside the principal slice locus. Indeed, it may happen that for some
$Z \in O_{\rho,\sigma}$ and $Z' \in O_{\rho',\sigma}$ there exists $g \in G_d$ such that
\[
\iota(Z) = g \cdot \iota(Z'),
\]
even though $g \notin H_{r,\sigma}$. In that case, $\iota(Z)$ and $\iota(Z')$ represent the same point of
$\mathcal C_r$, but $Z$ and $Z'$ are not related by the residual $H_{r,\sigma}$-action on $\Gamma_{d,XY}$.
Thus distinct rank strata in the slice can collapse to the same point, or the same $G_d$-orbit, in
$\mathcal C_r$.
\noindent
Geometrically, the obstruction is the possible presence of \emph{extra transporter symmetries}
\[
\mathrm{Trans}_{G_d}\!\bigl(\iota(Z), S_{r,\sigma}\bigr) \supsetneq H_{r,\sigma},
\]
...
identification
\[
\mathcal C_r^\circ \cong G_d \times_{H_{r,\sigma}} \Gamma^\circ_{d,XY}.
\]
Accordingly, the rank-stratification of $\Gamma^\circ_{d,XY}$ lifts faithfully to $\mathcal C_r^\circ$,
...
\[
\phi_0 = \sigma := \Sigma_{XY}U_Q : V_L \to V_0 .
\]
\noindent
...
\[
\Gamma_{d,XY}=
\Gamma_{d,\sigma}\big|_{\phi_l=Z_l \text{ for } l=1,\ldots,L}.
\]
\noindent
Equivalently, $\Gamma_{d,XY}$ is the space of cyclic quiver representations
...
\(r\in R_d^{\mathrm{orb}}\) satisfying \(\operatorname{rank}(r)=\operatorname{rank}(\sigma)\).
\end{theorem}
\noindent
...
\mathcal C_r^{(\rho)}
:=
\bar\Phi\!\bigl(G_h\times_{H_{r,\sigma}} O_{\rho,\sigma}\bigr)
=
G_h\cdot \iota(O_{\rho,\sigma}) \subseteq \mathcal C_r.
\]
...
$g\in G_h$ and $Z\in O_{\rho,\sigma}$ such that $W=g\cdot \iota(Z)$.
\end{definition}

\begin{remark}
In general, the subsets $\mathcal C_r^{(\rho)}$ need not be disjoint, since distinct rank strata in
$\Gamma_{d,XY}$ may map to the same critical point under $\bar\Phi$. Thus the natural global lift of the
rank-stratification of $\Gamma_{d,XY}$ is not a genuine partition of $\mathcal C_r$ by single rank patterns,
...
Let
\[
\bar\Phi \colon G_h \times_{H_{r,\sigma}} \Gamma_{d,XY} \longrightarrow \mathcal C_r
\]
...
\]
be the orbit stratification of $\Gamma_{d,XY}$ by cyclic rank patterns from Theorem~5.21.
\noindent
For each admissible cyclic rank pattern $\rho$, define the corresponding stack piece by
\[
\mathfrak X_\rho
:=
\bigl[\,G_h \times O_{\rho,\sigma}\,/\,H_{r,\sigma}\,\bigr].
\]
Equivalently, $\mathfrak X_\rho$ is the quotient groupoid associated to the restriction of the
right $H_{r,\sigma}$-action on $G_h\times \Gamma_{d,XY}$ to the invariant subset
$G_h\times O_{\rho,\sigma}$.
\noindent
We also write
\[
\mathfrak X
:=
\bigl[\,G_h \times \Gamma_{d,XY}\,/\,H_{r,\sigma}\,\bigr].
\]
Then the decomposition of $\Gamma_{d,XY}$ induces a decomposition
\[
\mathfrak X
=
...
$O_{\rho,\sigma}\subset \Gamma_{d,XY}$ are pairwise disjoint $G_\sigma$-orbits. Thus the ambiguity in
lifting rank strata from $\Gamma_{d,XY}$ to $\mathcal C_r$ does not occur at the level of the stack
...
Let
\[
\bar\Phi \colon X=(G_h\times \Gamma_{d,XY})/H_{r,\sigma}\longrightarrow C_r
\]
be the surjective map to the critical locus. For a saddle \(W\in C_r\), a
...
\[
W=g\cdot \iota(Z), \qquad g\in G_h,\; Z\in \Gamma_{d,XY},
\]
modulo the \(H_{r,\sigma}\)-equivalence.
\end{definition}

\begin{proposition}[Rank patterns distinguish different presentations]
For each saddle \(W\in C_r\), the fiber \(X_W\) admits a stratification labelled by cyclic rank patterns:
\[
X_W=\bigsqcup_{\rho} X_{W,\rho},
\qquad
X_{W,\rho}:=\{[g,Z]\in X_W \mid Z\in O_{\rho,\sigma}\},
\]
where the \(O_{\rho,\sigma}\subset \Gamma_{d,XY}\) are the strata indexed by
...
\\
\noindent
Therefore, the fibers of \(\bar\Phi\), together with their decomposition into
...
\operatorname{Supp}_{\mathrm{rk}}(W)
=
\{\rho \mid \exists\, [g,Z]\in G_h\times_{H_{r,\sigma}}\Gamma_{d,XY}
