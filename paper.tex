...
\eqref{eq:endpoints-slice}.
By construction, $\Gamma_{d,XY}\subset \mathcal Z$ is $H_{r,\sigma}$-stable (this is exactly the meaning of
...
\end{equation}
Then $\Phi(G_h \times \Gamma_{d,XY}) = C_r$ and $\Phi$ is invariant under the right action of $H_{r,\sigma}$ on $G_h \times \Gamma_{d,XY}$ given by
\begin{equation}\tag{24}
(g, Z) \cdot h := (g h,\; h^{-1} \cdot Z).
\end{equation}
Hence $\Phi$ descends to a surjective map
\[\
\bar\Phi \colon G_h \times_{H_{r,\sigma}} \Gamma_{d,XY} \longrightarrow C_r.
\]
...
\]
and let $C_r^{\circ} := \Phi(G_h \times \Gamma_{d,XY}^{\circ})$.
...
C_r^{\circ} \;\cong\; G_h \times_{H_{r,\sigma}} \Gamma_{d,XY}^{\circ}
...
C_r^{\circ} / G_h \;\cong\; \Gamma_{d,XY}^{\circ} / H_{r,\sigma}.
\end{equation}
\end{theorem}
\noindent
...
Fix $Q=C_{L+1}$ with vertices $Q_0=\{0,\dots,L\}$ and arrows $a_l:l-1\to l$ for $l=0,\dots,L$
with $a_0:L\to 0$.
\noindent
For $i,j\in Q_0$, let
\[
[i\rightsquigarrow j] \ \text{mod } L+1
\]
denote the oriented segment obtained by following the arrows of the cycle from $i$ to $j$.
\noindent
The \emph{interval module} $M(i,j)$ is the representation of $Q$ defined by
\[
M(i,j)_t :=
\begin{cases}
k, & t\in[i\rightsquigarrow j], \ \text{mod } L+1 \\ 
0, & \text{otherwise},
\end{cases}
\]
...
M(i,j)_{a_t}:=
\begin{cases}
\mathrm{id}_k, & \text{if both endpoints of }a_t\text{ lie in }[i\rightsquigarrow j],\\
0, & \text{otherwise}.
\end{cases}
\]
\end{definition}
...
\begin{lemma}[Uniseriality of $M(i,j)$]
For every interval module $M(i,j)$, the $A$-module $M(i,j)$ is uniserial, and in particular indecomposable.
\end{lemma}

\begin{proof}
Any nonzero submodule $N\subseteq M(i,j)$ must contain the one-dimensional space at some vertex in the support $[i\rightsquigarrow j]$.
Because all arrow maps along the support are identities, once $N$ contains the space at some vertex $t\in [i\rightsquigarrow j]$, it must also contain the spaces at all vertices in the terminal segment $[t\rightsquigarrow j]$.
Hence the nonzero submodules are exactly the interval modules
\[
M(t,j), \qquad t\in [i\rightsquigarrow j].
\]
These form a chain under inclusion. Therefore the submodules of $M(i,j)$ are totally ordered, so $M(i,j)$ is uniserial.
\end{proof}
...
\end{proof}
\begin{proposition}[Indecomposables of the cyclic truncated Nakayama algebra]
Let $A=kQ/I$ be the cyclic truncated Nakayama algebra associated with $Q=C_{L+1}$.
Then every finite-dimensional indecomposable $A$-module is isomorphic to an interval
module $M(i,j)$ for some $i,j\in Q_0$ with $\delta(i,j)\le L$. Equivalently, every indecomposable $A$-module is of the form $P(i)/J^{\ell}P(i)$ for a unique pair $(i,\ell)$.
In particular, $A$ has exactly $(L+1)^2$ indecomposable modules up to isomorphism.
\end{proposition}
\begin{comment}
...
Counting pairs $(i,\ell)$ with $i\in Q_0$ and $0\le \ell\le L$ gives $(L+1)^2$.
\end{proof}
...
\[
M \ \cong\ \bigoplus_{i=0}^{L} \ \bigoplus_{\ell=0}^{L} \ M(i,\ell)^{\oplus m_{i,\ell}}.
\]
\end{proposition}
...
\[
M(i,\ell), \qquad i \in \{0,\dots,L\}, \quad  0 \le \ell \le L,
\]
...
While the indecomposable representation of $\Gamma_{d,\sigma}$ are parameterized by the $M(i,\ell)$ from the previous modules over Nakayama algebras we are actually interested in stratifying:
\begin{equation*}
...
\begin{align*}
    F: \ & \Gamma_{d,L+1} \longrightarrow \Hom(V_L,V_0) \\
    & (\phi_0,\ldots,\phi_L) \longmapsto \phi_0.
\end{align*}
...
\[
\Gamma_{d,\sigma} \;:=\; F^{-1}(\sigma).
\]
Consider the stabilizer subgroup of $G_d$
\[
...
Consider the morphism of real varieties
\[
...
\medskip
\noindent
Consider the stratification of $\Gamma_{d,\sigma}$ into $G_d$--orbits indexed by a parameter set $\Lambda$,
\[
...
Given $\phi=(\phi_0,\dots,\phi_{L})\in\Gamma_d$, for any pair $(i,j)$ with $\delta(i,j)\le L$ we define the composed path map
\[
\Phi_{i\to j}(\phi)
:=
...
\Bigl\{
r\in R_d \ \Bigm|\ 
r_{i,\,i+\ell-1}-r_{i,\,i+\ell}\ge 0 \ \text{for all } i\text{ and } \ell=0,\dots,L
\Bigr\},
\]
where indices are taken modulo $L+1$.
\end{definition}
...
\begin{theorem}[Orbit stratification of $\Gamma_{XY}$ by cyclic rank patterns]\label{prop:GammaXY_orbit_stratification}
...
\begin{proof}
We proceed in explicit steps. Note that in this proof we have $\mathcal X_{r,\sigma}:= \Gamma_{d,XY}$ 
...
\end{proof}
\subsubsection*{Stratification of cyclic conditions with target map by rank patterns}
We prove theorem \ref{prop:GammaXY_orbit_stratification}
\begin{proof}\label{proof:GammaXY_orbit_stratification}
\medskip\noindent\textbf{Step 1: $G_d$--orbit decomposition of $\Gamma_d$.}
...
\medskip\noindent\textbf{Step 2: Passage to the fibre $\Gamma_{XY}$ and $G_\sigma$--orbits.}
Consider the morphism
\[
...
\medskip\noindent\textbf{Step 3: Reparametrization by cyclic rank patterns.}
...
\medskip\noindent\textbf{Step 4: The constraint $r(m)=\rank(\sigma)$ in rank-pattern coordinates.}
