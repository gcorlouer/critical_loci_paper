...
Let $\Sigma_{XY} \in \mathrm{Mat}_{d_0,\; d_L}$ and $U_Q \in \mathrm{Mat}_{d_L,\; d_L - r}$. Define
\[
\Gamma_d^r
:=
\left\{
...
\]
\end{definition}
\begin{definition}[Variety $\Sigma_r$]
We define
\[
\Sigma_r
:=
...
\end{definition}
\begin{definition}
Define the algebraic variety $$\Gamma_{d,XY}:= \Sigma_r\cap\Gamma_d^r $$ also defined by a set of vanishing polynomial cyclic conditions.
\end{definition}
\begin{comment}
...
\]
(Here $\sigma$ is the ``closing arrow'' in the later cyclic-quiver model.)
\begin{comment}
...
\end{comment}
\begin{proposition}
From the paper \cite{alchour}, we can write the set of rank $r$ critical points of a DLN as the following algebraic variety:
\begin{equation*}
    \mathcal{C}_r = \{W\in \Omega, Z_l\in\mathbb{R}^{d_l-r\times d_{l+1}-r}|W_L = [U_S, U_QZ_L],  W_l \sim \begin{pmatrix} I_r & 0 \\
                                                      0.  & Z_l      
                                        \end{pmatrix}, W_1 = [\Sigma_X^{-1}\Sigma_{XY}U_S, Z_1^{\top}]^{\top}, Z_l\in\Gamma_{d,XY}\}
\end{equation*} 
In other words, $\mathcal{C}_r \subset \Omega$ denote the set of rank-$r$ first-order critical points in the normal form of the DLN, i.e.\ the set described by the existence of residual blocks $Z = (Z_1, \dots, Z_L)$ such that
\[
...
\qquad Z \in \Gamma_{d,XY}
\]
This suggests that stratifying the geometry of the critical loci amounts to stratifying the geometry of $\Gamma_{d,XY}$.
\end{proposition}
%------------------------------------------------------------
\section{Critical loci as a bundle over $\Gamma$}
%------------------------------------------------------------
...
\mathcal S_{r,\sigma} \;:=\; \iota(\Gamma_{d,XY}) \;\subset\; \Omega .
\end{equation}
...
\]
act on $\Omega$ via \ref{eq:action-G_d} with $g_0=g_L=I$ (endpoints fixed).
...
The critical loci $\mathcal{C}_r$ is the orbit of the gauge-fixed critical slice $\mathcal{S}_{r,\sigma}$ by the hidden-layer gauge group $G_h$:
\begin{equation}\label{eq:orbit-saturation}
\mathcal{C}_r = G_h \cdot S_{r,\sigma}.
\end{equation}
\end{proposition}
\noindent
We prove this proposition in the appendix \ref{proof:orbit-saturation}
\begin{proposition}\label{prop:parameters-as-bundle}
    Define $\Phi \colon G_h \times \Gamma_{d,XY} \to \Omega$ by
\begin{equation}
\Phi(g, Z) := g \cdot \iota(Z),
\end{equation}
Then $\Phi(G_h \times \Gamma_{d,XY}) = \mathcal{C}_r$ and $\Phi$ is invariant under the right action of $H_{r,\sigma}$ on $G_h \times \Gamma_{d,XY}$ given by
\begin{equation}
(g, Z) \cdot h := (g h,\; h^{-1} \cdot Z).
\end{equation}
Hence $\Phi$ descends to a surjective map
\[\
\bar\Phi \colon G_h \times_{H_{r,\sigma}} \Gamma_{d,XY} \longrightarrow \mathcal{C}_r.
\]
With 
\begin{equation*}
     G_h \times_{H_{r,\sigma}} \Gamma_{d,XY} := (G_h \times \Gamma_{d,XY}) / H_{r,\sigma}
\end{equation*}
\end{proposition}
\noindent
...
\]
and let $\mathcal{C}_r^{\circ} := \Phi(G_h \times \Gamma_{d,XY}^{\circ})$.
\end{definition}
\begin{theorem}\label{thm:critical-bundle}
The restriction
\[
\bar\Phi \colon G_h \times_{H_{r,\sigma}} \Gamma_{d,XY}^{\circ}
\xrightarrow{\;\sim\;} \mathcal{C}_r^{\circ}
\]
is a bijection.  In particular, on the principal slice locus one has the identification:
\begin{equation}
\mathcal{C}_r^{\circ} \;\cong\; G_h \times_{H_{r,\sigma}} \Gamma_{d,XY}^{\circ}
...
\begin{equation}
\mathcal{C}_r^{\circ} / G_h \;\cong\; \Gamma_{d,XY}^{\circ} / H_{r,\sigma}.
\end{equation}
\end{theorem}
\noindent
...
\[
G_h \cdot \iota(O_{\rho,\sigma}) \cap G_h \cdot \iota(O_{\rho',\sigma}) = \varnothing
\qquad\text{whenever } \rho \neq \rho'.
\]
However, this need not hold outside the principal slice locus. Indeed, it may happen that for some
$Z \in O_{\rho,\sigma}$ and $Z' \in O_{\rho',\sigma}$ there exists $h \in G_h$ such that
\[
...
Thus distinct rank strata in the slice can collapse to the same point, or the same $G_h$-orbit, in
$\mathcal C_r$.
\noindent
Geometrically, the obstruction is the possible presence of \emph{extra transporter symmetries}
\[
\mathrm{Trans}_{G_h}\!\bigl(\iota(Z), S_{r,\sigma}\bigr) \supsetneq H_{r,\sigma},
\]
...
identification
\[
\mathcal C_r^\circ \cong G_h \times_{H_{r,\sigma}} \Gamma^\circ_{d,XY}.
\]
...
Identifying the residual parameters of a deep linear network with the quiver maps
\[
\phi_l = Z_l, \qquad l=0,\ldots,L,
\]
...
$V_l\cong\mathbb{R}^{d_l-r}$. Define the base--change group
\[
...
$\mathcal M_d^+(A)$.
\end{proposition}
\section{Stratification of vanishing cyclic conditions with fixed target map}
...
\]
modulo the natural \(H_{r,\sigma}\)-equivalence.
\end{definition}
